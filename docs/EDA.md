...
Итог: **97 ключей**, медиана 12 ключей на объявление, медиана длины значения 1 слово,
99.6% строк начинаются с ключа. Метод не идеальный: жёсткие пары вроде
«Тип стоимости за услугу / Начальная цена» энтропия не разделяет, потому что они всегда идут
вместе. Для фасетного матчинга и очистки текстового индекса этого хватает.
...
Второе следствие: дубли будут занимать слоты в топ-50. Пятьдесят почти одинаковых объявлений
в ответе это потерянная полнота, так что в конце нужна диверсификация.
...
при метрике на полноту. Поэтому схема другая:

1. фильтры дают не отсев, а сильные признаки и приоритет
2. для трети запросов, где после фильтров осталось меньше 50 кандидатов, отдаём их целиком
   и добираем оставшиеся слоты текстовым поиском по всему корпусу
3. для остальных ранжируем внутри отфильтрованного множества, но резервируем часть слотов
   под кандидатов снаружи, чтобы закрыть те самые 19.6%
